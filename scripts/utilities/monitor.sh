...
while true; do
    ...
    # Connection status
    if docker exec $CONTAINER test -f /mt4/MQL4/Files/connection_status.log 2>/dev/null; then
        conn=$(docker exec $CONTAINER cat /mt4/MQL4/Files/connection_status.log 2>/dev/null)
